...
TQL-11 maintains an internal state of 12 x 256-bit lanes. In v0.9.0, these are no longer stored in an array-index structure during the hot loop, but as discrete variables to force **Register Allocation**:
...
Previous versions suffered from *Register Spilling*, where the CPU would move state data to the Stack (RAM) due to the 16-register limit of AVX2. v0.9.0 refactors the core to use exactly 14 YMM registers:
...
To maintain integrity across non-aligned file sizes, v0.9.0 implements a tri-stage cleanup:
...
- **Single-Core Stability:** ~23.38 MiB/s (A +125% leap from v0.8.0).
- **Multi-Core (Parallel):** ~1.1 GiB/s (Utilizing Rayon for high-frequency auditing).
- **DPP Optimization:** Engineered for low-power consumption on edge devices by minimizing memory bus activity.

## 6. Conclusion
TQL-11 v0.9.0 stands as a benchmark for **Software-Defined High Performance**. By treating the CPU register file as the primary storage medium and implementing aggressive loop unrolling, Tequel provides a robust, fast, and cryptographically sensitive solution for the next generation of Digital Product Passports and narrative data integrity.